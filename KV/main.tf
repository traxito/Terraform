...
resource "azurerm_key_vault" "kv-alias" {
  ...
  access_policy {
   ...
   # ]
    ...
    secret_permissions = [
      "Get", "Backup", "Create", "Decrypt", "Delete", "Encrypt", "Import", "List", "Purge", "Recover", "Restore", "Sign", "Update", "Verify", "WrapKey", "Release", "Rotate"
    ]
...
resource "azurerm_monitor_diagnostic_setting" "diag-kv-alias" {
  ...
  storage_account_id = azurerm_storage_account.st-name.id
